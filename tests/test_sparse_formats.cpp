...
void test_csr_format() {
    ...
    CSR csr_weighted("src/graph.txt", true);
    csr_weighted.Print();
    
    // Test unweighted CSR
    std::cout << "\nTesting unweighted CSR:" << std::endl;
    CSR csr_unweighted("src/graph.txt", false);
...
void test_csc_format() {
    ...
    CSC csc_weighted("src/graph.txt", true);
    csc_weighted.Print();
    
    // Test unweighted CSC
    std::cout << "\nTesting unweighted CSC:" << std::endl;
    CSC csc_unweighted("src/graph.txt", false);
...
void test_coo_format() {
    ...
    COO coo_weighted("src/graph.txt", true);
    coo_weighted.Print();
    
    // Test unweighted COO
    std::cout << "\nTesting unweighted COO:" << std::endl;
    COO coo_unweighted("src/graph.txt", false);
...
void test_inheritance() {
    ...
    matrices[0] = new CSR("src/graph.txt", true);
    matrices[1] = new CSC("src/graph.txt", true);
    matrices[2] = new COO("src/graph.txt", true);
...
void test_error_handling() {
    ...
    try {
        CSR csr_error("non_existent_file.txt", true);
